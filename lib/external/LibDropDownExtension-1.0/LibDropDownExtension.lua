...
local function CustomDropDownButton_OnEnter(self)
  ...
  if option.hasArrow then
    ...
    if ( not listFrame or not listFrame:IsShown() or select(2, listFrame:GetPoint(1)) ~= self ) then
      ToggleDropDownMenu2(level, nil, nil, cdropdown, nil, nil, self.option.menuList, cdropdown);
    end
  else
...
function ToggleDropDownMenu2(level, value, dropDownFrame, anchorName, xOffset, yOffset, menuList, button, autoHideDelay)
  if ( not level ) then
    level = 1;
  end
